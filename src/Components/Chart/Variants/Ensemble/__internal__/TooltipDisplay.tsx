...
export default function TooltipDisplay({ day, view }: { day: number; view: ChartViews }) {
    ...
    if (hoverIndex > -1) {
        return (
            <>
                <h1 style={{ marginBottom: "5px" }}>
                    Avg: {trunc(dataPoints[hoverIndex].y[AVG_INDEX])}
                    {forecastUnit}
                </h1>
                <p>
                    Min {trunc(dataPoints[hoverIndex].y[MIN_INDEX])} | Max{" "}
                    {trunc(dataPoints[hoverIndex].y[MAX_INDEX])}
                </p>
            </>
        );
    } else if (day === 0) {
        return (
            <>
                <h1 style={{ marginBottom: "5px" }}>
                    Avg: {trunc(dataPoints[weather.nowIndex].y[AVG_INDEX])}
                    {forecastUnit}
                </h1>
                <p>
                    Min {trunc(dataPoints[weather.nowIndex].y[MIN_INDEX])} | Max{" "}
                    {trunc(dataPoints[weather.nowIndex].y[MAX_INDEX])}
                </p>
            </>
        );
    }
