...
test("Renders as expected", async () => {
    ...
    expect
        .soft(screen.queryByRole("slider")?.nextElementSibling?.children)
...
describe("Playback", () => {
    ...
    test("Playback loops to the beginning if the end is reached", async () => {
        ...
        expect.soft(screen.getByRole<HTMLInputElement>("slider").valueAsNumber).toBe(0);
    });
    ...
    test("A spinner is shown while the layer is loading preventing playback", async () => {
        ...
        expect
            .soft(screen.getByRole<HTMLInputElement>("slider").valueAsNumber)
