...
import { Thunder } from "Components/Background/__internal__";
...
describe("Visibility", () => {
    ...
    ] as WeatherCondition["type"][])("Shows if Condition is %s", async (condition) => {
        const renderer = await RTTR.create(<Thunder condition={condition} />)
    
        expect(
            renderer.scene.findByType("PointLight").props.visible
        ).toBe(true)
    })
