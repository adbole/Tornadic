...
import { Peek } from "Components/Radar/__internal__";



const mocks = vi.hoisted(() => ({ 
    popup: vi.fn() ,
    peek: vi.fn(({ isOpen }: { isOpen: boolean; }) => (
        <>
            {isOpen && <div role="dialog" />}
        </>
    )),
...
("%s, %s, %s", (event, moveCancel, upCancel, args) => {
    const fire = (map: L.Map) => {
        if(event === "mousedown") 
            map.fire(event, args)
        else
            fireEvent.touchStart(map.getContainer(), args)
    }
    ...
    test("After a second the modal appears", async () => {
        ...
        expect.soft(mocks.popup).toHaveBeenCalledWith(
            expect.objectContaining({ position: mocks.latLng }),
            {}
        )
        expect.soft(mocks.peek).not.toHaveBeenCalledWith(
        ...
        expect.soft(mocks.peek).toHaveBeenCalledWith(
            expect.objectContaining({ latitude: 35, longitude: -95 }),
            {}
        )
    })
    ...
    ("%s", (_, cancelMethod) => {
        ...
        act(() => {
            if(event === "mousedown")
                map.fire(cancelMethod)
            else
                fireEvent(map.getContainer(), new TouchEvent(cancelMethod))
