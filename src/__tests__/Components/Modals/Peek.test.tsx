...
vi.mock("Contexts/WeatherContext", () => ({
    default: ({ latitude, longitude, children, skeletonRender }: { 
        latitude: number, 
        longitude: number, 
        children: React.ReactNode,
...
vi.mock("Components/Simple", () => ({
    default: ({ icon, title, property }: { icon: React.ReactNode, title: string, property: string }) => (
        <div>
            <p>Icon: {icon}</p>
            <p>Title: {title}</p>
...
describe("WeatherContext", () => {
    ...
    test("Renders the Simple widgets with the correct props", () => {
        ...
        expect.soft(
            screen.getByText(simpleWidget("Droplet", "Precipitation", "precipitation"))
        ).toBeInTheDocument();

        expect.soft(
            screen.getByText(simpleWidget("Thermometer", "Dewpoint", "dewpoint_2m"))
        ).toBeInTheDocument()

        expect.soft(
            screen.getByText(simpleWidget("Moisture", "Humidity", "relativehumidity_2m"))
        ).toBeInTheDocument();

        expect.soft(
            screen.getByText(simpleWidget("Eye", "Visibility", "visibility"))
        ).toBeInTheDocument();
    })
