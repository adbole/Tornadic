...
test.each(
    ...
    ] as [ChartViews, keyof CombinedHourly][]
)("View %s has y2 property %s", async (view, y2Prop) => {
    function Wrapper({ children }: { children: React.ReactNode }) {
        return (
    ...
    const { result: { current: { dataPoints } } } = renderHook(useChart, { wrapper: Wrapper })
    const weather = useWeather.useWeather().weather

    expect(dataPoints.map(point => point.y2))
        .toStrictEqual(weather.getAllForecast(y2Prop).slice(0, 24))
})
...
test("View precipitation has a band scale", async () => {
    ...
    const { result: { current: { x } } } = renderHook(useChart, { wrapper: Wrapper })
