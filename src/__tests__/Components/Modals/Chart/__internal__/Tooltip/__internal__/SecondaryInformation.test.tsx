...
] as [ChartViews, Function][])
("Given view %s", (view, fn) => {
    beforeAll(() => {
...
("When given view %s, secondaryInformation is provided by %s", (view, property, label) => {
    test("When hoverIndex is -1 and day 0, secondaryInformation is current value", () => {
        ...
        expect(screen.getByText(`${label}: ${TooltipHelpers.trunc(value)}${unit}`)).toBeInTheDocument()
    })
    ...
    test("Day > 0 and hoverIndex = -1 uses getLowHigh", () => {        
        ...
        expect.soft(TooltipHelpers.getLowHigh).toHaveBeenCalledWith(expect.anything(), property, 1)
    })
