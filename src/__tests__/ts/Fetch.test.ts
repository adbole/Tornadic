...
describe("fetchData", () => {
    ...
    test("fail", async () => {
        ...
        await expect(fetchData("https://example.com", "Error message")).rejects.toBe("Error message")
    })
})
...
describe("fetchDataAndHeaders", () => {
    test("success", async () => {
        const fakeData = { data: '12345' };
        fetchMock.mockOnce(JSON.stringify(fakeData), { headers: { "Content-Type": "application/json" } });
    ...
    test("fail", async () => {
        ...
        await expect(fetchDataAndHeaders("https://example.com", "Error message")).rejects.toBe("Error message")
    })
})
