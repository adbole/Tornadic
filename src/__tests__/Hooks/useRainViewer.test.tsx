...
const renderRainViewer = () => 
    renderHook(useRainViewer, { wrapper: Wrapper });

const constructRadarPath = (path: string) => `${rainviewerObj.host}${path}/512/{z}/{x}/{y}/6/1_1.png`
const constructSatellitePath = (path: string) => `${rainviewerObj.host}${path}/512/{z}/{x}/{y}/0/1_1.png`
...
describe("Data Fetching", () => {
    test("Requests data from rainviewer", () => {
        ...
        expect.soft(fetchMock).toHaveBeenCalledWith("https://api.rainviewer.com/public/weather-maps.json");
    ...
    test("Fetches new data from rainviewer once the time is equal to nowcast", async () => {
        ...
        await act(async () => {
            await vi.advanceTimersByTimeAsync(rainviewerObj.radar.nowcast.slice(-1)[0].time * 1000 - Date.now())
        })
...
test("Once fetched, the map is prepared and availableLayers is provided", async () => {
    ...
    expect.soft(result.current.availableLayers?.Radar.currentLayerIndex)
        .toBe(rainviewerObj.radar.past.length - 1)
    expect.soft(result.current.availableLayers?.Satellite.currentLayerIndex)
...
describe("showFrame", () => {
    ...
    describe("Wrapping", () => {
        ...
        test("Wraps preload to beginning if next frame is out of range", async () => {
            ...
            expect.soft(L.tileLayer).toHaveBeenCalledTimes(2)
            expect.soft(L.tileLayer).toHaveBeenCalledWith(
                constructRadarPath(rainviewerObj.radar.nowcast[rainviewerObj.radar.nowcast.length - 1].path), 
                expect.anything()
            )
            expect.soft(L.tileLayer).toHaveBeenLastCalledWith(
                constructRadarPath(rainviewerObj.radar.past[0].path), 
